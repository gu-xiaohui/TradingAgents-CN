...
_mongo_db = _mongo_client['tradingagents']
...
async def get_rankings(
    ...
    try:
        ...
        # 查询数据
        stocks = list(_mongo_db['market_quotes'].find(
        ...
        stock_info = {}
        for info in _mongo_db['stock_basic_info'].find({'code': {'$in': stock_codes}}, {'_id': 0, 'code': 1, 'name': 1}):
        ...
        # 统计数据
        total = _mongo_db['market_quotes'].count_documents({'trade_date': latest_date})
        up_count = _mongo_db['market_quotes'].count_documents({'trade_date': latest_date, 'pct_chg': {'$gt': 0}})
        down_count = _mongo_db['market_quotes'].count_documents({'trade_date': latest_date, 'pct_chg': {'$lt': 0}})
        flat_count = _mongo_db['market_quotes'].count_documents({'trade_date': latest_date, 'pct_chg': 0})
        ...
        logger.error(f"[rankings] 获取数据失败: {e}", exc_info=True)
        return {
            "success": False,
